...
def eval_class(gt_annos,
    ...
    recall = np.zeros(
        [num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS*3])
    ...
    for m, current_class in enumerate(current_classes):
        for idx_l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                index += 1
                savedict = {}
                savedict['index'] = index
                savedict['metric'] = metric
                savedict['class'] = current_class
                savedict['difficulty'] = difficulty
                savedict['min_overlap'] = min_overlap
                savedict['pr'] = pr
                # prs.append(pr)
                # print(f'Index:{index}')
                ...
                # print('Precision:')
                # print(precision[m, idx_l, k, :])
                # print('Recall:')
                # print(recall[m, idx_l, k, :])
                savedict['precision'] = precision[m, idx_l, k, :]
                savedict['recall'] = recall[m, idx_l, k, :]
    ...
    # print(savelist)
    with open(f'res-pp_spl_{metric}.pkl','wb') as f:
